...
public class Controller {
	...
	public void redirectToMainPage(HttpServletRequest request, HttpServletResponse response, @PathVariable ("kullaniciId") String kullaniciId, @PathVariable ("sifre") String sifre)
	...
	public ResponseEntity<byte[]> avatarImageIn(HttpServletRequest request, HttpServletResponse response, @PathVariable ("kullaniciId") String kullaniciId, @PathVariable ("modul") String modul)
			throws IOException {
		if(kullaniciId != null && kullaniciId.length() != 0 && !kullaniciId.equalsIgnoreCase("anonim")){
			File file = new File(Constants.AVATAR_PATH + kullaniciId + ".jpg");
			response.setContentType("image/jpg");
